...
class MutRecRate(BaseSimulator):
    ...
    See the popgen-npe "Soup-to-Nuts" tutorial:
    ...
    default_config = {
        ...
        "pop_size": 10000,
        # RANDOM PARAMETERS (UNIFORM) -- [low, high] prior bounds
    ...
    def __call__(self, seed: int = None) -> (tskit.TreeSequence, np.ndarray):
        ...
        torch.manual_seed(seed)
        theta = self.prior.sample().numpy()
        recombination_rate, mutation_rate = theta

        # NOTE: the population must be named (a string, not an integer) so
        # that the prediction pipeline can match it against popmap.yaml
        demography = msprime.Demography()
